...
export interface ProcedureShape {
  readonly execute: (
    def: ProcedureDefinition<any, any, any, any, any>,
    initialContext: BaseContext,
    input: unknown,
  ) => Effect.Effect<any, any, any>
}

export class Procedure extends Context.Tag("@effect-trpc/Procedure")<Procedure, ProcedureShape>() {
  static readonly Live = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return {
        execute: (def, initialContext, input) =>
          Effect.gen(function* () {
            ...
            const handlerResult = service.handler(ctx as never, parsedInput as never) as unknown
            ...
            if (def.type === "stream" || def.type === "chat") {
              ...
              for (const provided of middlewareResult.services) {
                resultStream = Stream.provideService(resultStream, provided.tag, provided.value)
              }

              const encodedStream = def.outputSchema
                ? Stream.mapEffect(resultStream, (part) =>
                    Schema.encodeUnknown(def.outputSchema ?? Schema.Any)(part),
                  )
                : resultStream
            ...
            for (const provided of middlewareResult.services) {
              effectResult = Effect.provideService(effectResult, provided.tag, provided.value)
            }
