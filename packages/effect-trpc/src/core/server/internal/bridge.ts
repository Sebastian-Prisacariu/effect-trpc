...
export class RpcBridgeError extends Schema.TaggedError<RpcBridgeError>()("RpcBridgeError", {
...
export class RpcBridge extends Context.Tag("@effect-trpc/RpcBridge")<RpcBridge, RpcBridgeShape>() {
      ...
      return {
        toHttpLayer: (
          ...
          const group = RpcGroup.make(
            ...pipe(
              Array.fromRecord(rpcDefs),
              Array.map(([_name, rpc]) => rpc),
            ),
          )
