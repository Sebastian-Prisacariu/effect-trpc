...
export const make = handler => () => {
  ...
  return RequestResolver.makeBatched(requests => {
    ...
    const processEffects = pipe(Effect.forEach(effectRequests, _ => Effect.map(Schema.serialize(_.request), request => ({
      ..._,
      request
    ...
    const processStreams = pipe(Effect.forEach(streamRequests, request => {
      ...
      const stream = pipe(Schema.serialize(request.request), Effect.map(_ => ({
        ...request,
        request: _
