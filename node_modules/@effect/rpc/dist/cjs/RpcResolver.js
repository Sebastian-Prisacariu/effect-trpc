 ...
 */
const make = handler => () => {
  const getDecode = (0, _rpc.withRequestTag)(req => Schema.decodeUnknown(Schema.exitSchema(req)));
  const getDecodeChunk = (0, _rpc.withRequestTag)(req => Schema.decodeUnknown(Schema.Chunk(Schema.exitSchema(req))));
  return RequestResolver.makeBatched(requests => {
    ...
    const processEffects = (0, _Function.pipe)(Effect.forEach(effectRequests, _ => Effect.map(Schema.serialize(_.request), request => ({
      ..._,
      request
    ...
    const processStreams = (0, _Function.pipe)(Effect.forEach(streamRequests, request => {
      ...
      const stream = (0, _Function.pipe)(Schema.serialize(request.request), Effect.map(_ => ({
        ...request,
        request: _
...
//# sourceMappingURL=RpcResolver.js.map